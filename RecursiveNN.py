...
## 首先定义一个树节点结构，这样，我们就可以用它保存卷积神经网络生成的整棵树
...
class RecursiveLayer(object):
    ...
    # forward函数接收一系列的树节点对象作为输入，然后，递归神经网络将这些树节点作为子节点，并计算它们的父节点。
    # 最后，将计算的父节点保存在self.root变量中
    ...
    # calc_delta函数和calc_gradient函数分别计算各个节点的误差项以及最终的梯度。
    # 它们都采用递归算法，先序遍历整个树，并逐一完成每个节点的计算
