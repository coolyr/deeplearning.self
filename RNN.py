...
## 该类实现一个循环层 => [该部分的代码有点问题?!]
## 该类不包含参数U的更新方式,这部分实际上和全连接神经网络是一样的,感兴趣的同学可以自己来完成!
class RecurrentLayer(object):
    def __init__(self, input_width, state_width, activator, learning_rate):
        self.input_width = input_width # 3
        self.state_width = state_width # 2
        self.activator = activator # ReluActivator
        self.learning_rate = learning_rate # 0.001
        self.times = 0  # 当前时刻初始化为t0
        self.state_list = []  # 保存各个时刻的state
        self.state_list.append(np.zeros((state_width, 1)))  # 初始化s0 =[0 ... 0]
        self.U = np.random.uniform(-1e-4, 1e-4, (state_width, input_width))  # 初始化 U
        self.W = np.random.uniform(-1e-4, 1e-4, (state_width, state_width))  # 初始化 W
    ...
    def calc_delta(self, sensitivity_array, activator):
        ...
        for i in range(self.times): ## times = 2
            self.delta_list.append(np.zeros((self.state_width, 1)))
        self.delta_list.append(sensitivity_array) ## shape = <2, 1>;  size = 3
        # <逆向>迭代计算每个时刻的误差项
        for k in range(self.times-1, 0, -1): # k = 【1】
    ...
    def calc_delta_k(self, k, activator):
        ...
        delta[T转置，t] = delta[T转置，t+1] * W * f'(net[t])
        '''
        state = self.state_list[k + 1].copy()
        element_wise_op(self.state_list[k + 1], activator.backward)
        # print("calc_delta_k  k=",k)
        # print("state = ", state)
        # print("state[:, 0] = ", state[:, 0])
        # print("state.shape = ", state.shape)
        # print(" np.diag(state[:, 0]) = ", np.diag(state[:, 0]))
        #################################################################################################
        ## 有问题？？ f'(net[t]) 不是 f'(net[t+1]) 吧？？
    ...
    def calc_gradient(self):
        ...
        for t in range(self.times + 1): ## times = 2
            self.gradient_list.append(np.zeros((self.state_width, self.state_width)))
        for t in range(self.times, 0, -1): ## t = [2，1]
