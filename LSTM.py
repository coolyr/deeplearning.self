...
这是因为构造LSTM对象的时候，我们还不知道它未来是用于训练（既有forward又有backward）还是推理（只有forward）
根据LSTM前向计算和方向传播算法，我们需要初始化一系列矩阵和向量，这些矩阵和向量有两类用途，一类是用于保存模型参数，例如 ；Wf,Wi,Wo,Wc,bf,bi,bo,bc;
另一类是保存各种中间计算结果，以便于反向传播算法使用，它们包括ht,ft,it,ot,ct,c~t,delta[t],delta[ft],delta[it],delta[ot],delta[c~t]以及各个权重对应的梯度。
'''
